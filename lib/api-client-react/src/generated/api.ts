...
export const initializePayment = async (
  initPaymentBody: InitPaymentBody,
  options?: RequestInit,
): Promise<PaystackInitResponse> => {
...
export const getGetAdminAgentsUrl = (params?: GetAdminAgentsParams) => {
  ...
  return stringifiedParams.length > 0
    ? `/api/admin/agents?${stringifiedParams}`
    : `/api/admin/agents`;
};
...
export const getGetAdminAgentsQueryKey = (params?: GetAdminAgentsParams) => {
  return [`/api/admin/agents`, ...(params ? [params] : [])] as const;
};
...
>(
  ...
  const queryFn: QueryFunction<Awaited<ReturnType<typeof getAdminAgents>>> = ({
    signal,
  }) => getAdminAgents(params, { signal, ...requestOptions });
...
export const verifyAgent = async (
  id: string,
  verifyAgentBody: VerifyAgentBody,
  options?: RequestInit,
): Promise<User> => {
  return customFetch<User>(getVerifyAgentUrl(id), {
    ...
    method: "PATCH",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(verifyAgentBody),
  });
};
...
export const getAdjustAgentBalanceMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
...
export type AdjustAgentBalanceMutationResult = NonNullable<
  Awaited<ReturnType<typeof adjustAgentBalance>>
>;
 ...
 * @summary Adjust agent wallet balance
 */
export const useAdjustAgentBalance = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
